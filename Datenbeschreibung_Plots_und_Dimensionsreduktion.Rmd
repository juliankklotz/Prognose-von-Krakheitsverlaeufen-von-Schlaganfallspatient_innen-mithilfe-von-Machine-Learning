...
load("C:/Dokumente/Uni/7. Semester/Bachelorarbeit/namedData.Rdata")
library(dplyr)
library(cluster)
library(ggplot2)
library(psych)
...
Die Variablen i10001, und i9001-i9012 werden wir gesammelt untersuchen, mittels der R-Funktion "paste()".

```{r}
nameni90er <- rbind(namenDE[paste("i900",as.character(1:9),sep=""),],namenDE[paste("i90",as.character(10:12),sep=""),],namenDE["i10001",])
nameni90er
i90er <- cbind(datDE[,nameni90er$Datenbank])
str(i90er)
levels(datDE[,"i9006"])
```
...
Betrachten wir nun die NA-Rate, und statistischen Kennzahlen aller qualitativen Variablen.
...
qual_var_stats <- cbind(qual_var_names,c(1:16),c(1:16),c(1:16),c(1:16),c(1:16))
...
for(i in c(1:16)){
  ...
  qual_var_stats[i,6] <- paste(round(mean(na.omit(datDE[,qual_var_names[i,1]])%in% c("Unbekannt","extern durchgeführt","5","6"))*100,2),"%",sep ="")
  qual_var_stats[i,7] <- paste(levels(datDE[,qual_var_names[i,1]]),collapse = ', ')
}

names(qual_var_stats) <- c("Variablen Code","Variablen Beschreibung","Rate fehlender Werte","relative Häufigkeit von 'Ja'*\\'Im Wachzustand'\\'Männlich'\\0-2","relative Häufigkeit von 'Nein'\\'Im Schlaf'\\'Weiblich'\\3-4","relative Häufigkeit von 'Unbekannt'\\'extern durchgeführt'\\5-6","Kategorien") 
qual_var_stats
...
Es ist klar ersichtlich, dass die Variable i9008 (PAVK) mit 44.25% fehlenden Werten unter den Geohnheitsvariablen am meisten fehlende Werte hat.

Nun ist die Frage, ob die fehlenden Werte der Gewohnheits-Variablen innerhalb der selben Observationen sind. Hierfür suchen wir die Observationen, in denen keine der i90er Variablen NAs haben, und berechnen wiederum die NA-Rate.

```{r}
100*(234378 - nrow(na.omit(i90er)))/234278
```
...
ggplot() + geom_point(data=datDE_filt_alt, aes(x=jahr, y=Mittelwert_Alter, size=Anzahl))+ggtitle("Alter und Anzahl Patient*innen von 2003 bis 2021")+ylab("um 5% getrimmter Mittelwert des Alters")+xlab("Jahr")+theme_light()+ theme(plot.title = element_text(hjust = 0.5))
```
...
ggplot() + geom_point(data=alt_group_filt2, aes(x=i4020, y=Alter,color = i25001maxindex,alpha=maxwert,size=count))+
  #ggtitle("Grafik nach Alter, NIHSS\nund MRS bei Nachverfolgung")+
  ggtitle("")+
  xlab("Schweregrad des Schlaganfalles (NIHSS)")+ylab("Alter")+theme_light()+labs(
  colour = "Häufigster Gesundheitszustand\n(MRS) bei Nachverfolgung",
  size = "Anzahl der Patient*innen",
  alpha = "Proz. Anteil des\nhäufigsten MRS"
)+scale_alpha_continuous(breaks=c(0.5,1),labels=c("50%","100%"))+theme(legend.spacing=unit(0,"cm"))
...
```{r}
set.seed(3000)
cl_var <- cbind(i90er,alter = datDE[,"Alter"])
cl_var <- cl_var %>%
  filter(alter>=18,alter <=120)
...
for (i in 1:ncol(cor_alle)){
  colnames(cor_alle)[i] <- str_replace(colnames(cor_alle)[i],"Ja","")
  rownames(cor_alle)[i] <- str_replace(rownames(cor_alle)[i],"Ja","")
}
